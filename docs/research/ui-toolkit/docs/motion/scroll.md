...
scroll() binds animations to scroll progress, hardware-accelerated where supported.

Motion's 5.1kb `scroll()` function creates scroll animations in JavaScript, for effects like parallax, progress bars and scroll-driven storytelling.
...
As part of Motion's hybrid engine, `scroll` is able to run animations with the `ScrollTimeline`[ API](https://developer.mozilla.org/en-US/docs/Web/API/ScrollTimeline) where possible for optimal hardware-accelerated performance, removing scroll measurements, improving scroll synchronisation and ensuring animations remain smooth even under heavy CPI usage.

Live example: https://examples.motion.dev/js/parallax
...
`scroll()` can also accept animations created with the `animate()`[ function](https://motion.dev/docs/animate).
...
Live example: https://examples.motion.dev/js/scroll-fade
...
Live example: https://examples.motion.dev/js/scroll-pinning
