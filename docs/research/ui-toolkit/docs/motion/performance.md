...
How browsers render animations, and how to write ones that hit 120fps.
...
Creating new layers doesn't come for free. Each takes space on the GPU. So do so sparingly. 3D transforms ride the same compositor path, but establishing a 3D context promotes more elements than you might expect: [3D transforms and perspective](https://motion.dev/docs/3d-transforms) has the trade-off.

#### Use alternative styles

Second, you can try replacing styles will better-performing alternatives.

We've already seen that browsers are improving support for `filter` on the compositor. So instead of animating `boxShadow`, animate `filter`[ with the ](https://developer.mozilla.org/en-US/docs/Web/CSS/filter)`drop-shadow`[ function](https://developer.mozilla.org/en-US/docs/Web/CSS/filter):
...
Likewise, browsers are adding `clipPath` to the compositor. So instead of animating `borderRadius`, animate `clipPath`[ with the ](https://developer.mozilla.org/en-US/docs/Web/CSS/clip-path)`inset`[ function](https://developer.mozilla.org/en-US/docs/Web/CSS/clip-path):
